...
class Donkey(object):
    ...
    def _read_and_decode(filename_queue):
        ...
        return image, length, digits, letters
    ...
    def build_batch(path_to_tfrecords_file, num_examples, batch_size, shuffled):
        ...
        image, length, digits, letters = Donkey._read_and_decode(filename_queue)

        min_queue_examples = int(0.4 * num_examples)
        if shuffled:
            image_batch, length_batch, digits_batch, letters_batch = tf.train.shuffle_batch([image, length, digits, letters],
                                                                             batch_size=batch_size,
                                                                             num_threads=2,
                                                                             capacity=min_queue_examples + 3 * batch_size,
                                                                             min_after_dequeue=min_queue_examples)
        else:
            image_batch, length_batch, digits_batch, letters_batch = tf.train.batch([image, length, digits, letters],
                                                                     batch_size=batch_size,
                                                                     num_threads=2,
                                                                     capacity=min_queue_examples + 3 * batch_size)
        return image_batch, length_batch, digits_batch, letters_batch
